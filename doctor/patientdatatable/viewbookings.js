...
let getPatientData=async()=>{
    const collRef=await query(collection(db,'Patient'),where('nextAppointmentDoctorUID','==',uid),where('nextAppointmentStatus','==','confirmed'),orderBy('nextAppointmentDate'));
    ...
    addTableRow(`${firstName} ${lastName}`,nextAppt,regAt,x);
})
}
